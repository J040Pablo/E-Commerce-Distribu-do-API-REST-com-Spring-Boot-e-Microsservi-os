...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleOrderNotFoundException(RuntimeException ex, HttpServletRequest request) {
        logger.warn("Order not found at path={}: {}", request.getRequestURI(), ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleFeignNotFoundException(FeignException.NotFound ex, HttpServletRequest request) {
        logger.warn("Downstream not found at path={}: {}", request.getRequestURI(), ex.getMessage());
        return buildErrorResponse(HttpStatus.NOT_FOUND, "Not Found", "Recurso não encontrado no serviço downstream", request.getRequestURI());
    }
